...
impl ResultExport {
	pub fn init() {
		...
		Reflect::set(&window, &PROP_NAME_SUITE.into(), &JsValue::UNDEFINED)
			.unwrap();
		Reflect::set(&window, &PROP_NAME_TOTAL.into(), &JsValue::UNDEFINED)
			.unwrap();
	}
