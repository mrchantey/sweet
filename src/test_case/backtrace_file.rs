...
impl<'a> BacktraceFile<'a> {
    ...
    pub fn file_context(&self) -> io::Result<String> {
        ...
        for i in start..end {
            ...
            let prefix = String::from(if is_err_line { ">" } else { " " }).red();

            let buffer = line_number_buffer(curr_line_no);
            let line_prefix = String::from(format!("{}{}|", curr_line_no, buffer)).faint();
            ...
            if is_err_line {
                //TODO string length
                output.push_string(&format!("{}|", " ".repeat(2 + LINE_BUFFER_LEN)).faint());
                output.push_str(&" ".repeat(col_no));
                output.push_str_line(String::from("^").red().as_str());
            }
        }
        let prefix = String::from("at").faint();
        let file_loc = String::from(self.file_rel.to_str().unwrap_or("unkown file")).cyan();
        let line_loc = String::from(format!(":{}:{}", self.line, self.col)).faint();
