...
impl TestCase for TestCaseNative {
    ...
    fn path(&self) -> PathBuf {
        Path::new(self.file).to_forward_slash()
    }
    fn name(&self) -> &str {
        self.name
    }
...
impl TestCaseNative {
    pub fn split_funcs<'a>(funcs: impl IntoIterator<Item = &'a Self>) -> TestCaseNativeSplit<'a> {
        funcs
            .into_iter()
            .fold(TestCaseNativeSplit::<'a>::default(), |mut split, t| {
                match &t.func {
                    TestCaseNativeFunc::Sync(f) => split.syncs.push((t, f)),
                    TestCaseNativeFunc::Series(f) => split.series.push((t, f)),
                    TestCaseNativeFunc::Parallel(f) => split.parallels.push((t, f)),
                }
                split
            })
    }
